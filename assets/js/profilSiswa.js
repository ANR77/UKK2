function btnUbahPassword(id_siswa) {
    ...
    $('#ubah-password').append('<div id="verif-password" class="d-flex"><input type="password" class="form-control mr-1" id="password" placeholder="Masukkan password lama""><button  id="btn-submit" class="btn btn-success mx-1" onclick="verifikasi('+id_siswa+')">Submit</button><button id="" class="btn btn-secondary mx-1" onclick="batal()">Batal</button></div><small id="pass-salah" class="text-danger d-none mt-1"><i class="fas fa-exclamation-circle mr-1"></i> Password Salah</small>');
    $('#password').focus();
}
