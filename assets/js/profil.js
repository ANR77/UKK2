...
function verifikasi(id_petugas) {
    let value = $('#password').val();
    $.ajax({
        type: 'GET',
        url: 'http://localhost/UKK2/profil/cekPassword/' + id_petugas +'/'+value,
        async: true,
        dataType: 'json',
        success: function (data) {
            if (data) {
                ...
                $('#btn-submit').attr('onclick','ubahPassword('+data['id_petugas']+')').html('Simpan');
            } else {
...
function ubahPassword(id_petugas) {
    let value = $('#password').val();
    $.ajax({
        type: 'GET',
        url: 'http://localhost/UKK2/profil/ubahPassword/' + id_petugas +'/'+value,
        async: true,
        dataType: 'json',
        statusCode: {
            500: function () {
